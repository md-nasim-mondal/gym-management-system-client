...
import { bookClass, fetchMyBookings } from '@/redux/features/bookingSlice';
...
export default function BookingPage() {
  ...
  const { schedules, isLoading: scheduleLoading } = useSelector((state: RootState) => state.schedule);
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2">
          <Card>
            <CardHeader>
              <CardTitle>
                Available Classes for {selectedDate ? format(selectedDate, 'MMMM d, yyyy') : 'Today'}
              </CardTitle>
              <CardDescription>
              ...
              {scheduleLoading ? (
                ...
                <div className="space-y-4">
                  {filteredSchedules.map((schedule) => {
                    ...
                    const isFull = schedule.trainees.length >= schedule.maxTrainees;
                      ...
                      <Card key={schedule._id} className="overflow-hidden">
                        <div className="p-6">
                          <div className="flex justify-between items-start">
                            <div>
                              <h3 className="text-lg font-semibold">{schedule.classType}</h3>
                              <p className="text-sm text-muted-foreground">
                                {schedule.startTime} - {schedule.endTime}
                              </p>
                              <div className="mt-2">
                                <p className="text-sm">
                                  <span className="font-medium">Trainer:</span> {schedule.trainer.name}
                                </p>
                                <p className="text-sm">
                                  <span className="font-medium">Availability:</span> {schedule.trainees.length}/{schedule.maxTrainees} spots filled
                                </p>
                              </div>
                            </div>
                            <div>
                          ...
                          <div className="mt-4">
                            <Button
                              ...
                              disabled={isBooked || isFull || hasConflict || bookingInProgress === schedule._id}
                              className="w-full"
                            >
